...
class WeightedTradingSystem:
    ...
    def _initialize_thresholds(self):
        """初始化阈值"""
        return {
            'open_long': 0.65,    # 开多仓阈值
            'open_short': 0.65,   # 开空仓阈值
            'close_position': 0.35, # 平仓阈值
            'strong_signal': 0.80  # 强烈信号阈值
        }
...
class AIWeightOptimizer:
    ...
    def optimize_weights(self, performance_data):
        """优化权重配置"""
        if len(performance_data) < 20:  # 需要足够的数据
            return False
        ...
        if new_weights:
            ...
            self.optimization_history.append({
                ...
                'performance_data': performance_data[-10:]  # 保存最近10次表现
            })
            return True
    ...
    def _ai_adjust_weights(self, condition_stats):
        ...
        for condition, stats in condition_stats.items():
            ...
            if stats['win_rate'] > 0.6 and 0.2 < stats['frequency'] < 0.8:
                new_weights[condition] *= 1.2
            # 胜率低的条件降低权重
            elif stats['win_rate'] < 0.4:
                new_weights[condition] *= 0.8
...
class AITradingBot:
    ...
    def _execute_trade(self, symbol, side, signal_score, mtf, equity, regime):
        ...
        # 基于信号强度调整仓位大小
        position_multiplier = 0.5 + (signal_score * 0.5)  # 0.5-1.0倍
        ...
        position_size = self.calculate_position_size(symbol, 
            ...
            base_size * position_multiplier, mtf)
        ...
        stop_loss = price - atr * stop_atr if side == 'LONG' else price + atr * stop_atr
        take_profit = price + atr * tp_atr if side == 'LONG' else price - atr * tp_atr
        ...
        if order:
            ...
            LOG.info(f"✅ {symbol} {side} 开仓 - 信号强度: {signal_score:.2f}, "
                    f"仓位: {position_size:.2f} USDT,  regime: {regime}")
        ...
        for side in ['LONG', 'SHORT']:
            if side in pos_data and pos_data[side].get('units', 0) > 0:
                ...
                if should_close:
                    ...
                    if success:
                        # 记录交易结果
                        pnl = self.calculate_pnl(symbol, side, entry_price, current_price, 
                                               units, unit_usdt)
                        self.ai_optimizer.record_trade(symbol, side, entry_price, 
                                                     current_price, units, pnl, 
                                                     pos_data[side]['regime'], 
                                                     self.dynamic_params)
